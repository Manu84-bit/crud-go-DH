...
func (h *PatientHandler) DeleteById(ctx *gin.Context){
	idParam := ctx.Param("id")
	id, err := strconv.Atoi(idParam)
	if err != nil {
		ctx.AbortWithStatusJSON(http.StatusBadRequest, web.NewBadRequestApiError(err.Error()))
	...
	idFound, errFound := h.PatientService.DeletePatient(id)
	if errFound != nil {
		if apiError, ok := errFound.(*web.ErrorApi); ok {
			ctx.AbortWithStatusJSON(apiError.Status, apiError)
			return
...
func (h *PatientHandler) UpdatePatient(ctx * gin.Context){
	...
	if id == requestBody.Id {
	  idUpdated, err3 := h.PatientService.UpdatePatient(requestBody)
	  if err3 != nil {
		ctx.AbortWithStatusJSON(http.StatusBadRequest, web.NewBadRequestApiError(err.Error()))
		return
	  }
